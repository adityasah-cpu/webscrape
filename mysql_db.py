"""
MySQL Database Module for Job Portal
Stores jobs in MySQL database instead of in-memory cache
"""
...
def init_database():
    ...
    try:
        ...
        print("✓ Database 'job_portal' ready")
        conn.close()
...
def get_statistics():
    ...
    try:
        ...
        cursor.execute("""
            ...
            GROUP BY work_type
        """)
        by_work_type = {row['work_type']: row['count'] for row in cursor.fetchall()}

        # Jobs by country
        cursor.execute("""
            SELECT SUBSTRING_INDEX(country, ';', 1) as country, COUNT(*) as count
            FROM jobs
            GROUP BY SUBSTRING_INDEX(country, ';', 1)
            LIMIT 20
        """)
...
def get_last_fetch():
    ...
    try:
        ...
        return result['fetched_at'].isoformat() if result else None
